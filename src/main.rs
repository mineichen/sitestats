...
async fn count_ok(domain: web::Path<String>, state: web::Data<AppState>) -> Result<String, actix_web::Error> {
    let url = parse_url(domain.into_inner())?;
...
async fn count_problems(domain: web::Path<String>, state: web::Data<AppState>) -> Result<String, actix_web::Error> {
    let url = parse_url(domain.into_inner())?;
...
mod tests {
    ...
    async fn request_invalid_url_returns_400_status() {    
        ...
        let req = test::TestRequest::get().uri("/crawler/domain/i%nvalid/ok").to_request();
        let resp = test::call_service(&mut app, req).await;
        assert_eq!(400, resp.status());

        let req = test::TestRequest::get().uri("/crawler/domain/i%nvalid/ok/count").to_request();
        let resp = test::call_service(&mut app, req).await;
        assert_eq!(400, resp.status());

        let req = test::TestRequest::get().uri("/crawler/domain/i%nvalid/problems").to_request();
        let resp = test::call_service(&mut app, req).await;
        assert_eq!(400, resp.status());

        let req = test::TestRequest::get().uri("/crawler/domain/i%nvalid/problems/count").to_request();
        let resp = test::call_service(&mut app, req).await;
        assert_eq!(400, resp.status());
    }
}